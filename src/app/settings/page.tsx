...
import { toastMessage, passwordHash } from '@/utils/helpers';
...
export default function page() {
  ...
  const [debitFreeze, setDebitFreeze] = useState(false);
  ...
  const handleFreeze = async (data: TransactionPinFormInterface) => {
    if (!data.transaction_pin) {
      toastMessage('Transaction pin is required', '');
      return;
    }

    const freezeData: FreezeDebitDataInterface = {
      is_debit_frozen: !debitFreeze,
      transaction_pin: passwordHash(data.transaction_pin),
    };
    if (debitFreeze) {
      try {
        await userService.unfreezeDebit(freezeData);
        getData();
    ...
    if (!debitFreeze) {
      try {
        await userService.freezeDebit(freezeData);
        getData();
        setShowFreezeModal(false);
      } catch (error) {
        toastMessage(String(error), '');
  ...
  const getData = async () => {
    try {
      ...
      setDebitFreeze(response?.is_debit_frozen);
      setUser(response);
    } catch (error) {}
  };
          ...
          <div className="    flex gap-4 items-center ">
            <div className=" bg-grey py-[72px]  px-12 rounded-2xl  flex gap-12 flex-col min-w-[75%] mx-5 ">
              <div className=" text-center ">
                <h2 className=" text-purple text-[28px] font-semi-mid     ">
                  Confirming Debit{' '}
                  {currentUser?.is_debit_frozen ? 'Unfreeze' : 'Freeze'}
                </h2>
                <p className="  text-t-18  text-neutral-70   ">
                  {' '}
              ...
              <div>
                <form
                  onSubmit={methods.handleSubmit(handleFreeze)}
              ...
              <div className="  flex gap-12 ">
                ...
                <BtnMain
                  ...
                  btnStyle="  bg-blue  text-grey  flex-1"
                  onClick={() => handleFreeze(methods.getValues())}
                  type="submit"
                />
              </div>
            </div>
          ...
          <div className="flex flex-col gap-[35px]  ">
            {settings.map((setting, index) => (
              ...
              >
                ...
                <div className="">
                  {setting.title == 'Freeze' ? (
                    <Image
                      src={`/icons/${
                        debitFreeze ? 'switch-on' : 'switch-off'
                      }.svg`}
                      width={24}
                      height={24}
                      alt=""
